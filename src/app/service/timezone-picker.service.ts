...
export class TimezonePickerService {
    ...
    getTimezoneForCountryISO(iso: string) {
        return _.map(zones[iso], timeZone => {
            return {
    ...
    getTimezoneForCountry(cnt: string) {
        let tz;
        _.forEach(countryList, (country, key) => {
            if (country.includes(cnt)) {
                tz = key;
            }
        });
        return _.map(zones[tz], timeZone => {
            return {
                label: timeZone, value: timeZone
            }
        })
    ...
    getCountryForTimeZone(timeZone: string) {
        let country;
        _.forEach(zones, (zone, key) => {
            if (zone.join(',').indexOf(timeZone) > -1) {
    ...
    getCountryISOForTimeZone(timeZone: string) {
        let country;
        _.forEach(zones, (zone, key) => {
            if (zone.join(',').indexOf(timeZone) > -1) {
                country = key;
